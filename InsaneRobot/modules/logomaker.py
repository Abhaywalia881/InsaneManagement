...
from InsaneRobot import OWNER_ID, SUPPORT_CHAT
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./InsaneRobot/resources/LOGOS/*")
    ...
    fname = "logo.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"**Made by @Insane_Help**")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report [Insane Server](t.me/Dosto_ki_Mehfil786), {e}')
